...
class PowerUp {
    ...
    use(game) { }
    update_state(delta_time, game) { }
    desc() { return "None"; }
}
;
function use_super(power_up, game) {
    if (power_up.power_up_cool_down <= 0 && power_up.power_up_count_down >= 0) {
        power_up.use(game);
    }
}
function update_state_super(power_up, dt, game) {
...
class PowerUpExtraPoints extends PowerUp {
    ...
    use(game) { }
    update_state(delta_time, game) { game.score += delta_time * 2; }
    desc() {
        return "Extra Points";
    }
}
class PowerUpRandomBall extends PowerUp {
    ...
    update_state(delta_time, game) { game.score += delta_time; }
    desc() {
...
class Game extends SquareAABBCollidable {
    ...
    update_state(delta_time) {
        ...
        for (let i = 0; i < this.balls.length; i++) {
            ...
            const destroy_ball = ball.bounce(this.x, this.y, this.width, this.height) || ball.mid_x() + ball.radius < 0 || ball.mid_x() - ball.radius > this.width ||
        ...
        this.collision_map.handle_by_cell((paddle, paddle2) => {
        }, (paddle, brick) => {
            ...
            if (ball.radius && collision_code) {
                ...
                if (ball.radius && (brick).hp > 0 && brick !== this.paddle) {
                    ball.snap_back_out_of_squareAABBColidable(brick, collision_code, delta_time);
